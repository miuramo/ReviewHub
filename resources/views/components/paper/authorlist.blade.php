...
@php
    $koumoku = \App\Models\Paper::including_optional_bibs(); //書誌情報
@endphp
...
<div class="mx-6 my-0">
    ...
    <form action="{{ route('paper.update_authorlist', ['paper' => $paper]) }}" method="post" id="authorlist">
        ...
        <div class="mx-4 mb-1">
            <label for="authors"
                class="block text-sm font-medium text-gray-900 dark:text-gray-400">サンプルを参考に、一行に一人ずつ、氏名のあいだに半角スペースをいれて入力してください。所属に(株)などの括弧は含めないでください。
                複数所属のときは/で区切ってください。部署や部局、研究室名を入力する必要はありません。</label>
            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                <div class="p-0">
                    <div class="text-sm px-2  dark:text-gray-400">{{ $koumoku['authorlist'] }}</div>
                ...
                <div class="p-0">
                    <div class="text-sm px-2  dark:text-gray-400">{{ $koumoku['authorlist'] }} サンプル</div>
                ...
                <div class="p-0">
                    <div class="text-sm px-2  dark:text-gray-400">{{ $koumoku['eauthorlist'] }}（姓→名の順で記す場合は、姓のあとに, (カンマ)を入れることが多いです。）</div>
                ...
                <div class="p-0">
                    <div class="text-sm px-2  dark:text-gray-400">{{ $koumoku['eauthorlist'] }} サンプル</div>
            ...
            <x-element.submitbutton color="blue" :value="1">
                著者名と所属を更新
