...
@elseif($task->workflow->task == 'approve')
    <x-review.commentpaper_link :sub="$task->submit" label="総合判定結果"></x-review.commentpaper_link>
...
@elseif($task->workflow->task == 'submit')
    @php
        $rev = App\Models\Review::where('submit_id', $task->submit->id)->where('user_id', $task->subject->id)->first();
    ...
    @isset($rev->start_at)
        @if ($rev->status == 2)
            <div class="bg-cyan-100 px-3 py-2">
            @else
        ...
        <span class="mx-2"></span>
        @if ($rev->status == 2)
            <span class="inline-block border-2 border-blue-600 px-2 py-1 text-blue-600 font-bold text-sm">
                必須項目入力済
            </span>
            <span class="mx-1"></span>
            <form action="{{ route('task.update', ['task' => $task]) }}" method="post" class="inline-block">
                @csrf
                @method('PUT')
                <input type="hidden" name="task" value="{{ $task->id }}">
        ...
        @if ($rev->target > 0)
            ...
            <span class="mx-1"></span>
            <x-element.linkbutton2
                href="{{ route('paper.review', ['sub' => $sub->id, 'token' => $sub->paper->token()]) }}" color="purple"
        ...
        @endif
        {{-- ファイル一覧 --}}
        <x-review.filelist_for_review :paper_id="$paper->id"></x-review.filelist_for_review>
        <div class="mx-2 my-2"></div>
        </div>
    @else
        {{-- まだ開始していない --}}
        <div class="mx-2 bg-orange-100 px-3 py-2">
