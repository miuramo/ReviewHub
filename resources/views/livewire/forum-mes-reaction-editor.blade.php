<div class="relative flex flex-wrap items-center gap-1 mt-1" x-data="{ open: false }"
    ...
    @foreach ($reactions as $reaction)
        <button type="button" wire:key="reaction-{{ $forumMesId }}-{{ $reaction['emoji'] }}"
    ...
    <button type="button" wire:key="picker-toggle-{{ $forumMesId }}" @click="open = !open"
    ...
    <div x-show="open" x-cloak @mouseleave="open = false" wire:key="picker-popover-{{ $forumMesId }}"
        ...
        @foreach ($emojiGroups as $groupIndex => $group)
            <div class="flex flex-wrap gap-1 mb-1" wire:key="picker-group-{{ $forumMesId }}-{{ $groupIndex }}">
                @foreach ($group as $emoji)
                    <button type="button" wire:key="picker-emoji-{{ $forumMesId }}-{{ $emoji }}" @click="open = false"
