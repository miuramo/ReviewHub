<x-app-layout>
    ...
    @php
        $acceptbbs = \App\Models\Bb::bb_accepted(); //著者との掲示板一覧
    @endphp
        ...
        <div class="px-6">
            ...
            @foreach ($bufary as $n => $ba)
                @if (!isset($titleary[$ba['PID']]))
                    <div class="bg-red-100 dark:bg-red-300">PaperID: {{ sprintf('%03d', $ba['PID']) }} は存在しません。</div>
                    @continue
                @endif
                @php
                    $ba['title'] = $titleary[$ba['PID']];
                    $ba['PID'] = sprintf('%03d', $ba['PID']);
                @endphp
    ...
    <div class="py-2 px-6">
        <div class="p-2 bg-lime-100 dark:bg-lime-600">
            該当PaperIDの掲示板が作成されていない場合は、自動的に作成せず、スキップします。
        </div>
    </div>
    <div class="py-2 px-6">
