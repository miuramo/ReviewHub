...
class ViewpointController extends Controller
{
    ...
    public function itmsetting(Request $req)
    {
        ...
        if ($req->has('copy_id')) {
            ...
            $newdatum->orderint++;
            $newdatum->save();
            return redirect()->route('viewpoint.itmsetting', ['cat_id' => $enqitm->category_id, 'cat_name' => $req->input('cat_name')])->with('feedback.success', '行をコピーしました');
        }
        // del_id がセットされていたら、行を削除する
        if ($req->has('del_id')) {
            $del_id = $req->input('del_id');
            Viewpoint::destroy($del_id);
            return redirect()->route('viewpoint.itmsetting', ['cat_id' => $req->input('cat_id'), 'cat_name' => $req->input('cat_name')])->with('feedback.success', '行を削除しました');
        }
        $coldetails = AdminController::column_details($tableName);
        $coldetails['COPY'] = 'COPY';
        $ary = ['COPY', 'orderint', 'name', 'desc', 'content', 'contentafter', 'target', 'mandatory', 'weight', 'doReturn', 'doReturnAcceptOnly','subdesc'];
        $cold2 = [];
        foreach ($ary as $f) {
