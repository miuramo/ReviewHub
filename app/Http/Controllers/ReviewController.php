...
class ReviewController extends Controller
{
     ...
     * for test (dummy)
     */
    public function edit_dummy($cat_id, $target = 1)
    {
    ...
    public function req_confirm_post(Request $req, Review $review, string $token)
    {
        ...
        if ($action == "accept") {
            // ここで、各種処理を行う
            $ret = $review->do_accept(); //すでに2名いれば、falseになる
            if ($ret) {
                $review->do_assign(); // 割り当てる
                $message = $review->message_accept_by_maillink();
            } else {
                $message = "査読をお引き受けいただき、ありがとうございます。<br>" .
                    "ご回答いただいたところ誠に恐縮ですが、現在、ほかの査読者の方からもほぼ同時に申し出をいただいたため、調整をしております。<br>" .
                    "調整のあとで、改めて状況のご連絡、または査読のお願いをさせていただきます。<br>" .
                    "（査読を辞退される場合は、お手数ですが再度メールのボタンからご回答をお願いします。）<br>" .
                    "ご多忙のところご検討・ご回答いただき、誠にありがとうございました。";
            }
        } elseif ($action == "deny") {
