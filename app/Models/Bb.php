...
class Bb extends MetaModel
{
    ...
    public static function make_bb(Submit $sub, int $type = 1, int $rev_id = 0): Bb
    {
        ...
        $firstmes = [
            1 => "ここは{$name_of_managers}と著者の掲示板です。絵文字リアクション機能を用いると、リアクションをつけたユーザの氏名が表示されます。",
            2 => "ここは{$name_of_managers}と査読者の掲示板です。他の査読者の方はメンバーに含まれていません。絵文字リアクション機能を用いると、リアクションをつけたユーザの氏名が表示されます。",
            3 => "ここは{$name_of_managers}と全査読者の掲示板です。\n査読者は自身を名乗らないでください。必要があればRevIDを用いてください。RevIDは送信フォームに表示されています。\n（RevIDが表示されていない場合は、査読を担当していません。）\n注：RevIDは査読者のIDではなく、査読割当てごとに異なるIDです。絵文字リアクション機能を用いると、リアクションをつけたユーザの氏名が表示されます。",
            4 => "ここは{$name_of_managers}同士の掲示板です。\n査読者はメンバーに含まれていません。絵文字リアクション機能を用いると、リアクションをつけたユーザの氏名が表示されます。",
        ];
        $bb = Bb::firstOrCreate([
